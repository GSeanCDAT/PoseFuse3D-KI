...
from wan.utils.logging_ import main_print
...
def save_checkpoint(wrapped_model, rank, output_dir, step):
    ...
    with FSDP.state_dict_type(
        ...
        cpu_state = wrapped_model.state_dict()
    transformer_state = {k: v for k, v in cpu_state.items() if k.startswith('transformer')}
    control_state = {k: v for k, v in cpu_state.items() if k.startswith('control_model')}
    
    # todo move to get_state_dict
    if rank <= 0:
        # import pdb;pdb.set_trace()
        # trainable_keys = {name for name, param in transformer.named_parameters() if param.requires_grad}
        # cpu_state = {k: v for k, v in cpu_state.items() if k in trainable_keys}
        # NOTE: only save the trainable part
        # transformer_state = {k: v for k, v in transformer_state.items() if "attn1.to_out" in k or 'attn1.to_v' in k}
        transformer_state = {k: v for k, v in transformer_state.items() if "attn1.to_out" in k}
        save_dir = os.path.join(output_dir, f"checkpoint-{step}")
        os.makedirs(save_dir, exist_ok=True)
        # save using safetensors
        weight_path = os.path.join(save_dir, "diffusion_pytorch_model.safetensors")
        control_model_weight_path = os.path.join(save_dir, "control_model.safetensors")
        save_file(control_state, control_model_weight_path)
        save_file(transformer_state, weight_path)
        config_dict = dict(wrapped_model.transformer.config)
...
def separately_save_checkpoint(transformer, rank, output_dir, step, control_model=None):
    main_print(f"--> saving checkpoint at step {step}")
    with FSDP.state_dict_type(
            transformer,
            StateDictType.FULL_STATE_DICT,
    ...
    if control_model is not None:
        with FSDP.state_dict_type(
            ...
            cpu_state_cm = control_model.state_dict()
    ...
    if rank <= 0:
        ...
        if control_model is not None:
            control_model_weight_path = os.path.join(save_dir, "control_model.safetensors")
            save_file(control_model.state_dict(), control_model_weight_path)
        save_file(cpu_state, weight_path)
...
# save wrapped fdsp model: lora weights of the transformer; control model
def save_lora_checkpoint(wrapped_model, optimizer, rank, output_dir, step, pipeline):
    with FSDP.state_dict_type(
            wrapped_model,
            StateDictType.FULL_STATE_DICT,
            FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
    ...
    with FSDP.state_dict_type(
        ...
        transformer_state = wrapped_model.transformer.state_dict()

    if wrapped_model.control_model is not None:
        with FSDP.state_dict_type(
                wrapped_model.control_model,
                StateDictType.FULL_STATE_DICT,
                FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
        ):
            control_state = wrapped_model.control_model.state_dict()
    ...
    if rank <= 0:
        ...
        transformer_lora_layers = get_peft_model_state_dict(model=wrapped_model.transformer, state_dict=transformer_state)
        ...
        lora_config = {
            "step": step,
            "lora_params": {
                "lora_rank": wrapped_model.transformer.config.lora_rank,
                "lora_alpha": wrapped_model.transformer.config.lora_alpha,
                "target_modules": wrapped_model.transformer.config.lora_target_modules,
            },
        }
